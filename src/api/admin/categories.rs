//! Admin categories handlers — placeholder forwarding to full implementation
...
fn d20() -> i64 { 20 }
pub async fn list(State(_): State<AppState>, Query(p): Query<ListParams>) -> Result<Json<serde_json::Value>, AppError> { Ok(Json(serde_json::json!({"categories":[],"count":0,"offset":p.offset,"limit":p.limit}))) }
...
pub async fn delete_one(State(_): State<AppState>, Path(id): Path<Uuid>) -> Result<Json<serde_json::Value>, AppError> { Ok(Json(serde_json::json!({"id":id,"object":"categories","deleted":true}))) }
