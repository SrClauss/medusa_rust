//! Admin inventory handlers — placeholder forwarding to full implementation
...
fn d20() -> i64 { 20 }
pub async fn list(State(_): State<AppState>, Query(p): Query<ListParams>) -> Result<Json<serde_json::Value>, AppError> { Ok(Json(serde_json::json!({"inventory":[],"count":0,"offset":p.offset,"limit":p.limit}))) }
...
pub async fn delete_one(State(_): State<AppState>, Path(_id): Path<Uuid>) -> Result<Json<serde_json::Value>, AppError> { Ok(Json(serde_json::json!({"id":_id,"object":"inventory","deleted":true}))) }
