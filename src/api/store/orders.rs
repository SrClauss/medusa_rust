...
pub struct OrderParams {{ pub cart_id: Option<Uuid>, pub display_id: Option<i32>, pub email: Option<String> }}
fn order_json(r: &sqlx::postgres::PgRow) -> serde_json::Value {{
    serde_json::json!({{
        ...
        "items": [], "shipping_methods": [], "payments": [], "fulfillments": [],
        "subtotal": 0, "tax_total": 0, "shipping_total": 0, "discount_total": 0, "total": 0,
    }})
}}
...
pub async fn get_orders_batch(State(_): State<AppState>, Query(_): Query<OrderParams>) -> Result<Json<serde_json::Value>, AppError> {{
