...
pub async fn list_orders(State(state): State<AppState>, axum::Extension(auth): axum::Extension<AuthCustomer>, Query(p): Query<OrdersParams>) -> Result<Json<serde_json::Value>, AppError> {
    let id: Uuid = auth.0.sub.parse().unwrap_or_default();
    let orders = sqlx::query("SELECT id, status, display_id, currency_code, created_at, updated_at FROM orders WHERE customer_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3")
        .bind(id).bind(p.limit).bind(p.offset).fetch_all(&*state.db).await?
        .into_iter().map(|r| serde_json::json!({"id":r.get::<Uuid,_>("id"),"status":r.get::<String,_>("status"),"display_id":r.get::<i32,_>("display_id"),"currency_code":r.get::<String,_>("currency_code"),"created_at":r.get::<chrono::DateTime<chrono::Utc>,_>("created_at"),"updated_at":r.get::<chrono::DateTime<chrono::Utc>,_>("updated_at"),"items":[],"total":0})).collect::<Vec<_>>();
